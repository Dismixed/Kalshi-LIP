...
def test_manage_orders_cancels_duplicates_and_places_when_needed(bot_factory):
    ...
    api.set_orders(ticker, [
        {"ticker": ticker, "side": side, "action": "buy", "yes_price": bid},
        {"ticker": ticker, "side": side, "action": "buy", "yes_price": to_tick(bid - 0.01)},
        {"ticker": ticker, "side": side, "action": "sell", "yes_price": ask},
    ])
